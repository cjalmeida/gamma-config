...
from .plugins import PROJECT_NAME, TagException, plugin_manager, application_tags
...
def reset_tags():
    from gamma.config.config import _config_store
...
def get_tags(blacklist=True) -> Dict[str, Callable]:
    ...
    if not hasattr(_config_store, "tags"):
        blacklist_tags = set()
        if blacklist:
        ...
        for tag_spec in itertools.chain(*plugins, application_tags):
            ...
            _config_store.tags[tag] = handler
    return _config_store.tags
